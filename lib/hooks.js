import { writeFileSync } from "fs";
import { join } from "path";

export function createHookHandlers({ historyManager, recentHistory, promptBuilder, workerDir, log, topicDetector, sharedState, onRestart, sessionManager, MAX_SESSION_SIZE }) {
  return {
    "session-start": async (data) => {
      log("hook", `session-start data keys: ${Object.keys(data || {}).join(", ")}`);
      if (data?.session_id) {
        sharedState.mainSessionId = data.session_id;
        log("hook", `stored mainSessionId: ${data.session_id}`);
      } else if (data?.sessionId) {
        sharedState.mainSessionId = data.sessionId;
        log("hook", `stored mainSessionId: ${data.sessionId}`);
      }
      // Commit pending plan if this is a "clear" session-start within 3min of ExitPlanMode
      let planAccepted = false;
      if (sharedState.pendingPlan) {
        const age = Date.now() - sharedState.pendingPlan.timestamp;
        const source = data?.hook_event_name || data?.source || "";
        if (age < 180000) {
          historyManager.appendToHistory({ type: "plan_accepted", plan: sharedState.pendingPlan.plan });
          planAccepted = true;
          log("hook", `committed pending plan (age=${age}ms, source=${source})`);
        } else {
          log("hook", `discarded pending plan (age=${age}ms, source=${source})`);
        }
        sharedState.pendingPlan = null;
      }
      const prompt = promptBuilder.buildSystemPrompt({ minimal: planAccepted || sharedState.justRestarted === "topic" });
      try { writeFileSync(join(workerDir, "last-system-prompt.log"), prompt); } catch {}
      return { status: 200, headers: { "Content-Type": "text/plain" }, body: prompt };
    },

    "user-prompt-submit": async (data) => {
      log("hook", `user-prompt-submit: ${data.prompt ? data.prompt.slice(0, 200) : "(no prompt)"}`);
      if (data.prompt) {
        let isNewTopic = false;
        if (sharedState.justRestarted) {
          sharedState.justRestarted = null;
          log("hook", "skipped post-restart prompt");
          return { status: 200 };
        }
        try {
          isNewTopic = await topicDetector.detectTopicChange(data.prompt);
          if (isNewTopic) {
            recentHistory.clear();
          }
        } catch {}
        historyManager.appendToHistory({ type: "user_prompt", prompt: data.prompt });
        recentHistory.append({ type: "user_prompt", prompt: data.prompt });

        // Check main session JSONL size → trigger restart if too large
        const mainJsonl = sharedState.mainSessionId ? sessionManager.findSessionJsonl(sharedState.mainSessionId) : null;
        if (mainJsonl && mainJsonl.size > MAX_SESSION_SIZE && onRestart) {
          log("hook", `main agent jsonl too large (${Math.round(mainJsonl.size / 1024)}KB > ${MAX_SESSION_SIZE / 1024}KB), restarting with user prompt`);
          onRestart(data.prompt, "size");
        } else if (isNewTopic && onRestart) {
          onRestart(data.prompt, "topic");
        }
      }
      return { status: 200 };
    },

    "exit-plan-mode": async (data) => {
      const plan = data.tool_input?.plan;
      log("hook", `exit-plan-mode: ${plan ? plan.slice(0, 200) : "(no plan)"}`);
      if (plan) {
        sharedState.pendingPlan = { plan, timestamp: Date.now() };
        log("hook", "saved pending plan, waiting for session-start clear to confirm");
      }
      return { status: 200 };
    },

    "post-ask-user-question": async (data) => {
      const questions = data.tool_input?.questions;
      const response = data.tool_response;
      log("hook", `post-ask-user-question: ${JSON.stringify(questions?.map(q => q.question))}`);
      if (questions) {
        historyManager.appendToHistory({
          type: "ask_user_question",
          questions: questions.map(q => q.question),
          answers: response,
        });
      }
      return { status: 200 };
    },

    "post-task-create": async (data) => {
      const input = data.tool_input || {};
      const response = data.tool_response;
      log("hook", `post-task-create: ${input.subject}, response=${JSON.stringify(response)}`);
      if (input.subject) {
        // Extract task ID from response object or string
        let taskId;
        if (response && typeof response === "object" && response.task) {
          taskId = response.task.id;
        } else {
          const responseStr = typeof response === "string" ? response : JSON.stringify(response);
          const match = responseStr && responseStr.match(/Task #(\d+)/);
          if (match) taskId = match[1];
        }
        historyManager.appendToHistory({
          type: "task_create",
          taskId,
          subject: input.subject,
          description: input.description,
        });
      }
      return { status: 200 };
    },

    "post-task-update": async (data) => {
      const input = data.tool_input || {};
      log("hook", `post-task-update: task=${input.taskId} status=${input.status}`);
      if (input.taskId) {
        historyManager.appendToHistory({
          type: "task_update",
          taskId: input.taskId,
          status: input.status,
          subject: input.subject,
        });
      }
      return { status: 200 };
    },

    "subagent-pretool": async (data) => {
      const input = data.tool_input || {};
      let prompt = input.prompt;
      if (prompt && prompt.length > 20000) {
        prompt = undefined;
      }
      log("hook", `subagent-pretool: ${input.subagent_type} / ${input.description}`);
      historyManager.appendToHistory({ type: "subagent_start", subagent_type: input.subagent_type, description: input.description, prompt });

      // Check main session JSONL size → trigger restart if too large
      const mainJsonl = sharedState.mainSessionId ? sessionManager.findSessionJsonl(sharedState.mainSessionId) : null;
      if (mainJsonl && mainJsonl.size > MAX_SESSION_SIZE) {
        log("hook", `main agent jsonl too large (${Math.round(mainJsonl.size / 1024)}KB > ${MAX_SESSION_SIZE / 1024}KB), restarting with "continue"`);
        onRestart("continue", "size");
      }

      return { status: 200 };
    },

    "subagent-stop": async (data) => {
      log("hook", `subagent-stop: ${data.agent_type || "(no agent_type)"}`);
      if (data.agent_type) {
        let msg = data.last_assistant_message;
        if (msg && msg.length > 20000) {
          msg = undefined;
        }
        historyManager.appendToHistory({ type: "subagent_stop", agent_id: data.agent_id, agent_type: data.agent_type, last_assistant_message: msg });
      }
      return { status: 200 };
    },

    "stop": async (data) => {
      log("hook", `stop: message length=${data.last_assistant_message?.length || 0}`);
      if (data.last_assistant_message && data.last_assistant_message.length <= 20000) {
        historyManager.appendToHistory({ type: "assistant_result", message: data.last_assistant_message });
        const msg = data.last_assistant_message.length > 300 ? data.last_assistant_message.slice(0, 300) + "..." : data.last_assistant_message;
        recentHistory.append({ type: "assistant_result", message: msg });
      }
      return { status: 200 };
    },
  };
}
