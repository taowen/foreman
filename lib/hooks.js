...
export function createHookHandlers({ historyManager, recentHistory, promptBuilder, workerDir, log, topicDetector, sharedState, onRestart }) {
  return {
    "session-start": async (data) => {
      ...
      // Commit pending plan if this is a "clear" session-start within 30s of ExitPlanMode
      ...
      const prompt = promptBuilder.buildSystemPrompt({ minimal: planAccepted || !!sharedState.justRestarted });
