...
export function createPromptBuilder(pluginDir, historyManager, log) {
  ...
  function buildSystemPrompt({ minimal = false } = {}) {
    ...
    if (total > 0) {
      ...
      if (minimal) {
        ...
        for (let i = recentStart; i < total; i++) {
          ...
          else if (e.type === "task_create") parts.push(`[TASK_CREATE id=${e.taskId}] ${e.subject}: ${e.description}`);
          else if (e.type === "task_update") parts.push(`[TASK_UPDATE id=${e.taskId}] status=${e.status}${e.subject ? ` subject=${e.subject}` : ""}`);
        }
      }
    ...
    if (hasWebSearch || hasWebFetch) {
      ...
      if (hasWebFetch) parts.push('use the "web-fetch" MCP tool to fetch and extract content from URLs');
