...
---`;
        ...
        if (isBedrock) {
          ...
          // Use Anthropic REST API (with proxy support)
          ...
          const { status: haikuStatus, text: haikuText } = await new Promise((resolve, reject) => {
          ...
          try {
            ...
            log("tool:web-fetch", `Haiku response not JSON (status ${haikuStatus}): ${haikuText.slice(0, 200)}`);
