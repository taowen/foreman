...
export function createWebFetchHandler(pluginDir, log) {
  return async (data) => {
    ...
    try {
      ...
      try {
        const haikuPrompt = `Given the markdown below, write a JavaScript expression that extracts content matching the instruction.
Available: \`markdown\` (string), \`parseMarkdown(markdown)\` returns { sections: [{level, heading, content}], section(query) }.
Return a string. If no relevant content, return "not found".
...
---`;
        ...
        // Prefill assistant response so Haiku only completes the expression after "return"
        const assistantPrefill = "return";
